...
from sellor.account.models import Address
from sellor.checkout import views
from sellor.checkout.forms import CartVoucherForm, CountryForm
...
def test_view_checkout_index(
    ...
    monkeypatch.setattr(
        'sellor.checkout.utils.get_cart_from_request', lambda req, qs: cart)
...
@patch('sellor.checkout.views.summary.send_order_confirmation')
...
def test_get_discount_for_cart_product_voucher_not_applicable(monkeypatch):
    monkeypatch.setattr(
        'sellor.checkout.utils.get_prices_of_discounted_products',
        lambda cart, product: [])
    voucher = Voucher(
...
def test_get_discount_for_cart_collection_voucher_not_applicable(monkeypatch):
    monkeypatch.setattr(
        'sellor.checkout.utils.get_prices_of_products_in_discounted_collections',  # noqa
        lambda cart, product: [])
    voucher = Voucher(
