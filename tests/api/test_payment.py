...
from sellor.graphql.payment.enums import (
    OrderAction, PaymentChargeStatusEnum, PaymentGatewayEnum)
from sellor.payment.models import ChargeStatus, Payment, TransactionKind
...
def test_payment_void_gateway_error(
    ...
    variables = {'paymentId': payment_id}
    monkeypatch.setattr(
        'sellor.payment.gateways.dummy.dummy_success', lambda: False)
    response = staff_api_client.post_graphql(
        VOID_QUERY, variables, permissions=[permission_manage_orders])
    content = get_graphql_content(response)
    data = content['data']['paymentVoid']
...
@patch('sellor.graphql.payment.resolvers.gateway_get_client_token')
