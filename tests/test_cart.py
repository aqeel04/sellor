...
from sellor.checkout import forms, utils
from sellor.checkout.context_processors import cart_counter
...
from sellor.checkout.views import clear_cart, update_cart_line
from sellor.core.exceptions import InsufficientStock
from sellor.core.utils.taxes import ZERO_TAXED_MONEY
from sellor.discount.models import Sale
from sellor.shipping.utils import get_shipping_price_estimate
...
def local_currency(monkeypatch):
    def side_effect(price, currency):
        return price
    monkeypatch.setattr('sellor.checkout.views.to_local_currency', side_effect)
...
def test_get_or_create_cart_from_request(
    ...
    monkeypatch.setattr(
        'sellor.checkout.utils.get_or_create_user_cart', mock_get_for_user)
    monkeypatch.setattr(
        'sellor.checkout.utils.get_or_create_anonymous_cart_from_token',
...
def test_get_cart_from_request(
    ...
    monkeypatch.setattr(
        'sellor.checkout.utils.get_user_cart', mock_get_for_user)
    ...
    mock_get_for_anonymous = Mock(return_value=anonymous_cart)
    monkeypatch.setattr(
        'sellor.checkout.utils.get_anonymous_cart_from_token',
        mock_get_for_anonymous)
    request = cart_request_factory(user=None, token=token)
    returned_cart = utils.get_cart_from_request(request, queryset)
    mock_get_for_user.assert_called_once_with(customer_user, queryset)
...
def test_cart_counter(monkeypatch):
    monkeypatch.setattr(
        'sellor.checkout.context_processors.get_cart_from_request',
...
def test_check_product_availability_and_warn(
    ...
    monkeypatch.setattr(
        'sellor.checkout.utils.contains_unavailable_variants',
    ...
    monkeypatch.setattr(
        'sellor.checkout.utils.remove_unavailable_variants',
...
def test_replace_cartline_form_when_insufficient_stock(
    ...
    monkeypatch.setattr(
        'sellor.product.models.ProductVariant.check_quantity',
