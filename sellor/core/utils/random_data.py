...
def create_products_by_schema(placeholder_dir, create_images):
    path = os.path.join(
        settings.PROJECT_ROOT, 'sellor', 'static', 'populatedb_data.json')
    with open(path) as f:
...
class SellorProvider(BaseProvider):
    def money(self):
        return Money(
...
fake.add_provider(SellorProvider)
...
# fake customers.
@patch('sellor.order.emails.send_payment_confirmation.delay')
...
def create_page():
    ...
    <p>Sellor is a rapidly-growing open source e-commerce platform that has served high-volume companies from branches like publishing and apparel since 2012. Based on Python and Django, the latest major update introduces a modular front end with a GraphQL API and storefront and dashboard written in React to make Sellor a full-functionality open source e-commerce.</p>
    <p><a href="https://github.com/aqeel04/sellor">Get Sellor today!</a></p>
    """
    content_json = {
        'blocks':
        [{
             ...
             'inlineStyleRanges': []},
         {
             ...
             'data': {},
             'text':
             'Sellor is a rapidly-growing open source e-commerce platform that has served high-volume companies from branches like publishing and apparel since 2012. Based on Python and Django, the latest major update introduces a modular front end with a GraphQL API and storefront and dashboard written in React to make Sellor a full-functionality open source e-commerce.',
             'type':
             'unstyled',
             'depth':
             0,
        ...
        'entityMap': {
            '0': {
                'data': {
                    'href': 'https://github.com/aqeel04/sellor'},
                'type': 'LINK',
                'mutability': 'MUTABLE'}}}
    page_data = {
